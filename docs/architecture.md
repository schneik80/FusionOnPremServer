...
- **Icons** — FontAwesome by default, routed by kind through `components/icons.ts` and rendered by the `ItemIcon` funnel in `components/entityIcons.tsx`, which substitutes hand-drawn marks for the kinds that have them. Custom glyphs paint in `currentColor` so they theme like an FA glyph; the two exceptions are `FusionLogo.tsx` and the Fusion design-intent marks in `components/intentIcons.tsx`, which are fixed-colour shaded artwork and therefore pick a light or dark drawing off `theme.palette.mode`. See [`docs/icons/STATUS.md`](icons/STATUS.md).
