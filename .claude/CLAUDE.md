...
- `internal/apsbudget/` — **the APS quota layer** every upstream call goes through (`api/budget.go` wires it into `gqlQueryAt` / `dmDo`): a points token bucket (the 6000/min app quota, resynced by real 429s), a strict-priority scheduler (P0 user-blocking → P1 in-viewport rows → P2 background) with reserve floors and bounded waits, a cooldown after real 429s, and a seconds-only coalescing cache whose key **always carries the user's subject** unless the op is registered `Shared` in `api/cost.go` (ACL-free hub facts only). Typed `RateLimitError` / `QueryTooComplexError`; `server/priority.go` maps `X-FLS-Priority` / route → priority and reports `X-FLS-Throttle` on every response; `GET /api/quota`. See `docs/quota/STATUS.md`.
...
- **APS calls are quota'd (6000 points/min per app, shared by every user), so never fan out per row and never bypass the budget.** Every upstream call goes through `gqlQueryAt` / `dmDo` (never `httpClient` directly), which take a slot from `internal/apsbudget`; a new query is registered in `api/cost.go` with its calibrated cost and TTL (`Shared` only for ACL-free hub facts — the coalescing key otherwise carries the subject). A 429 is the typed `RateLimitError`: not retried, it trips a cooldown and reaches the SPA as `Retry-After` + `retryAfterMs`; a fan-out that skips per-node errors must **abort** on one (`api.IsRateLimited`), never return a silently shorter result. Priority rides on the context (route table in `server/priority.go`, `X-FLS-Priority`, `?p=` on images; background jobs set P2 themselves). Client side, every APS-backed call carries a priority (`api/priority.ts`; hook defaults in `api/queries.ts`, `?p=` on image URLs via `thumbnailSrc`); anything per-item — a classify, a thumbnail, a card (`DocumentCard`, `PinnedDocCard`, `ProductionCard`, `NavRow`) — waits for the row to near the viewport via `components/useInView.ts`; anything per-container is capped with a visible "Load all" (`ACTIVITY_CAP` / `CLASSIFY_CAP` in `Dashboards.tsx`, `THUMB_CAP` in `relationCap.ts`). `active` flows from `BrowserStage` (what is on screen), never a literal `true`. A 429 is retried once after the server's `retryAfterMs` (`api/queryDefaults.ts`, shared with the embed entry); `refetchOnReconnect` is off except for realtime keys. The one slow-mode surface is `QuotaBanner` fed by `state/quota.ts` (headers on every response + 429s + `/api/quota` while hot) — never per-widget alerts. Never cap silently.
