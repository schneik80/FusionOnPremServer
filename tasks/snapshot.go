...
// Snapshot streams every project's tasks.json to visit for the backup engine,
// holding each project's mutex while that project's file is read so a backup
// never captures a mid-mutation file. rel paths are
// "tasks/<projectdir>/tasks.json". Project dirs whose file is unreadable or
// unrecognizable are skipped (one bad project must not abort the whole
// backup); an error from visit aborts the run.
func (s *Store) Snapshot(visit func(rel string, data []byte, schemaVersion int) error) error {
	...
	if err != nil {
		...
		return fmt.Errorf("tasks: scanning store dir: %w", err)
	}
	for _, e := range entries {
		if !e.IsDir() {
			continue
		}
		path := filepath.Join(s.dir, e.Name(), "tasks.json")
		...
		if err := snapshotOne(ps, path, "tasks/"+e.Name()+"/tasks.json", visit); err != nil {
			return err
		}
	}
	return nil
